...
namespace edgeflow
  ...
  begin
    ...
    have h : ∀ᵐ z : ℝ ∂μ, z ∈ set.interval_oc (θ + τ) (θ + (f.q θ)/ν + τ) → f.fm z = (λ θ, ν) z 
      := sorry,
    have hνpos : ν ≠ 0 
      := sorry,
    show ∫ z in 0..θ + f.c θ, f.fm z ∂μ = f.Fp θ,
    ...
    show f.Fm (θ + τ) + ∫ z in θ + τ..θ + f.c θ, f.fm z ∂μ = f.Fp θ,
    show f.Fm (θ + τ) + ∫ z in θ + τ..θ + (τ + (f.q θ) / ν), f.fm z ∂μ = f.Fp θ,
    rw add_comm τ _,
    rw ← add_assoc,
    rw interval_integral.integral_congr_ae h,
    simp,
    show f.Fm (θ + τ) + ∫ (x : ℝ) in θ + τ..θ + f.q θ / ν + τ, ν = f.Fp θ,
    rw interval_integral.integral_const,
    calc f.Fm (θ + τ) + (θ + f.q θ / ν + τ - (θ + τ)) • ν 
